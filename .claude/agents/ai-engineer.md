...
## Versions are 1.x — verify before you write

The project's real dependencies (`pyproject.toml`): **`langchain ≥1.2`, `langgraph ≥1.0`, `langgraph-checkpoint-postgres ≥3.0`, `langchain-mcp-adapters ≥0.2`, `langsmith ≥0.4.13`**, plus provider packages `langchain-{openai,anthropic,mistralai,ollama,google-genai,azure-ai,qdrant,postgres}`.
...
- RAG: `RecursiveCharacterTextSplitter` → embeddings → vector store. Per-silo backend (PGVector or Qdrant), collections `silo_{id}`, HNSW indexes.
- MCP: `langchain-mcp-adapters` `MultiServerMCPClient`; Mattin acts as both MCP server and client.
- **Deep Agents** patterns (on LangGraph 1.x) are available via the `deep-agents-*` skills even though the package isn't yet a repo dependency — use them only if a step calls for it.

## Project landmarks

- `backend/services/agent_execution_service.py` — the agent chat execution flow (file attachments → conversation → LangGraph chain → output parser → persist).
- `backend/tools/ai/` — LLM provider implementations. `backend/tools/embeddingTools.py` — embedding provider factory. `backend/tools/vector_store_factory.py` — per-silo PGVector/Qdrant. `backend/tools/langsmith_config.py` — per-app + global LangSmith tracing.
...
Provide a **change summary** and any `## Terminal Commands Required`. **Do not run git** — the orchestrating command commits behind confirmation gates.
