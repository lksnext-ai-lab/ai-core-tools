...
5. Capture genuine ambiguities as **Open Questions** rather than inventing answers — and ask the user the blocking ones.
6. Register the spec in `.claude/specs/index.yaml` (status `draft`/`refining`).

## Mattin AI context to apply

- Multi-tenant: everything scoped to an **App**; roles VIEWER→EDITOR→ADMINISTRATOR→OWNER→OMNIADMIN.
...
- Do not design the implementation — that is `solution-architect`'s job. Keep "Implementation Notes" to high-level architectural hints only.
