...
- **API surfaces**: `/public/v1` and `/mcp/v1` use X-API-KEY with rate limiting + CORS validation â€” confirm they're enforced. CORS never `*` in production.
...
6. **SSRF**: user-controlled URLs in server-side requests (web scraping/domains, MCP configs) â€” validate.
