...
1. Read `spec.md` fully. Verify any library/API assumptions against Context7 (and LangChain Docs for AI work) — the stack is **LangChain ≥1.2 / LangGraph ≥1.0, Pydantic v2, SQLAlchemy 2.x, React 19**.
2. Record **Architecture Decisions** (AD-N): the chosen approach, rationale, and alternatives rejected. Respect the project's layered architecture and existing patterns — reuse over reinvention.
3. Decompose into ordered steps. Each step:
   ...
   - is **self-contained** (the expert must not need to read the spec — embed all context, file paths, and patterns),
...
- Backend: router (HTTP only) → service (business logic) → repository (data) → model. DI via `Depends(get_db)`. RBAC `@require_min_role`. Never business logic in routers; never raw SQL.
