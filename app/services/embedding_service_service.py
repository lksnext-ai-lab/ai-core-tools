...
class EmbeddingServiceService:
    ...
    def delete_embedding_service(embedding_service_id):
        embedding_service = db.session.query(EmbeddingService).filter(EmbeddingService.embedding_service_id == embedding_service_id).first()
        if embedding_service:
            db.session.delete(embedding_service)
            db.session.commit()

    @staticmethod
    def delete_by_app_id(app_id):
        embedding_services = db.session.query(EmbeddingService).filter(EmbeddingService.app_id == app_id).all()
