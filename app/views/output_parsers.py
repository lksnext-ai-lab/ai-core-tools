...
@output_parsers_blueprint.route('/app/<app_id>/output-parsers', methods=['GET'])
def app_output_parsers(app_id):
    parsers = parser_service.get_parsers_by_app(app_id)
    return render_template('output_parsers/output_parsers.html', 
                         parsers=parsers, 
                         app_id=app_id)

@output_parsers_blueprint.route('/app/<app_id>/output-parser/<parser_id>', methods=['GET', 'POST'])
def app_output_parser(app_id, parser_id):
    if request.method == 'POST':
        ...
        parser_service.create_or_update_parser(int(parser_id), data)
        return app_output_parsers(app_id)

    parser = parser_service.get_parser_by_id(int(parser_id)) if parser_id != '0' else None
...
@output_parsers_blueprint.route('/app/<app_id>/output-parser/<parser_id>/delete', methods=['GET'])
def app_output_parser_delete(app_id, parser_id):
    parser_service.delete_parser(int(parser_id))
    return app_output_parsers(app_id) 