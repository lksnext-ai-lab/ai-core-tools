...
resume attempts, output truncation, and phaseful Skill activation.
...
def _healthy_for_sandbox(state: dict | None, sandbox_id: str) -> bool:
    ...
    return phases.get("files") == "ok" and phases.get("bootstrap") in ("ok", "skipped")
...
class DaytonaProvider(SandboxProvider):
        ...
        try:
            ...
            if hasattr(fs, "search_files"):
                ...
                for path in files or []:
                    ...
                    if rel and not rel.startswith(".skills/") and rel != ".skills":
                        result.append(rel)
    ...
    def ensure_skill(
        ...
        files_dir = f"{_workspace_root()}/.skills/{skill.name}"
        state: dict[str, Any] = {
