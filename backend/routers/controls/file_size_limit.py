...
async def enforce_file_size_limit(
    ...
    try:
        ...
        max_total_mb = max_file_size_mb  # Use the actual file size limit
        
        # Check if total payload exceeds the limit
        if content_length_mb > max_total_mb:
            ...
            raise HTTPException(
                ...
                detail=f"Total upload size ({content_length_mb:.2f}MB) exceeds maximum allowed ({max_total_mb}MB). Maximum {max_file_size_mb}MB per file."
            )
