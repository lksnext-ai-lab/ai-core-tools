...
These endpoints are only available when DEVELOPMENT_MODE is enabled.
"""
...
@router.post(
    ...
    description=(
        ...
        "Only works when DEVELOPMENT_MODE=true and email exists in database."
    ),
)
async def dev_login(
    ...
    - Only enabled when DEVELOPMENT_MODE=true
    ...
    if not DEVELOPMENT_MODE:
        logger.warning(
            "Dev login attempted while DEVELOPMENT_MODE is disabled"
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=(
                "Development login is not available. "
                "This endpoint only works when DEVELOPMENT_MODE is enabled."
            ),
        )
