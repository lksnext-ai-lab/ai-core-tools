...
def get_current_user_dev(
    ...
    the user in the database. Used only when OIDC_ENABLED=false.
...
if not OIDC_ENABLED:
    logger.info("Using development mode authentication (OIDC_ENABLED=false)")
    get_current_user_oauth = get_current_user_dev
else:
    logger.info("Using OIDC authentication (OIDC_ENABLED=true)")
