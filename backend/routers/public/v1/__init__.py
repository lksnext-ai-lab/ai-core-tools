...
public_v1_router.include_router(agents_router, prefix="/app/{app_id}/agents", dependencies=[Depends(enforce_app_rate_limit), Depends(enforce_allowed_origins)])
public_v1_router.include_router(chat_router, prefix="/app/{app_id}/chat", dependencies=[Depends(enforce_app_rate_limit), Depends(enforce_file_size_limit), Depends(enforce_allowed_origins)])
public_v1_router.include_router(files_router, prefix="/app/{app_id}/files", dependencies=[Depends(enforce_app_rate_limit), Depends(enforce_file_size_limit), Depends(enforce_allowed_origins)])
