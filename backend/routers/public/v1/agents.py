...
def _validate_referenced_resources(db: Session, agent_dict: dict, app_id: int):
    """
    Validate that service_id, silo_id, and output_parser_id belong to the same app.
    Raises HTTPException 400 if any referenced resource belongs to a different app.
    """
    service_id = agent_dict.get("service_id")
    if service_id:
        ...
        if not service:
            raise HTTPException(
                ...
                detail="Invalid service_id: not found in this app",
            )

    silo_id = agent_dict.get("silo_id")
    if silo_id:
        ...
        if not silo:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid silo_id: not found in this app",
            )
    ...
    if output_parser_id:
        ...
        if not parser:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid output_parser_id: not found in this app",
            )
...
async def create_agent(
    ...
    try:
        ...
        _validate_referenced_resources(db, agent_dict, app_id)
...
async def create_ocr_agent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Failed to create OCR agent",
        )
...
async def update_agent(
    ...
    existing_agent = validate_agent_ownership(db, agent_id, app_id)
    ...
    try:
        ...
        _validate_referenced_resources(db, update_dict, app_id)
...
async def update_ocr_agent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Failed to update OCR agent",
        )
