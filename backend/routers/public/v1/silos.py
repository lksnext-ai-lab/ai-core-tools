...
async def count_docs_in_silo(
    ...
    try:
        ...
        logger.error(f"Error counting documents in silo {silo_id}: {str(e)}")
...
async def index_multiple_documents(
    ...
    try:
        ...
        return MessageResponseSchema(message=f"Successfully indexed {len(documents)} document(s)")
    except Exception as e:
        logger.error(f"Error indexing multiple documents in silo {silo_id}: {str(e)}")
...
async def delete_all_docs_in_collection(
    ...
    try:
        ...
        logger.error(f"Error deleting all documents in silo {silo_id}: {str(e)}")
